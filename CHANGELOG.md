...
- Kartenkacheln standardmäßig von Esri (`server.arcgisonline.com`, World Street Map) statt direkt von `tile.openstreetmap.org` geladen: OSM blockt mittlerweile viele Browser-/App-Aufrufe ohne eigenen konformen User-Agent mit HTTP 403 „Access blocked" (osm.wiki/Blocked). Der zunächst probierte CARTO-Ausweg verlangt inzwischen selbst einen API-Key ("API KEY REQUIRED"-Wasserzeichen) und war ungeeignet. Neue Karten-Option `map_attribution`, `map_tiles` bleibt überschreibbar.
